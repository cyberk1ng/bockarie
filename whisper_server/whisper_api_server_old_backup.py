...
def preprocess_audio(audio_path: str) -> str:
    ...
    try:
        ...
        logger.info(f"Loaded audio: {len(audio)} samples at {sr}Hz, duration: {len(audio)/sr:.2f}s")

        # Resample to 16kHz (Whisper's optimal sample rate)
        if sr != 16000:
            audio = librosa.resample(audio, orig_sr=sr, target_sr=16000)
            logger.info(f"Resampled to 16kHz: {len(audio)} samples")

        # Normalize audio
        audio = librosa.util.normalize(audio)

        # Trim silence (use 40 dB for voice - less aggressive)
        audio_before_trim = len(audio)
        audio, _ = librosa.effects.trim(audio, top_db=40)
        logger.info(f"Trimmed audio: {audio_before_trim} -> {len(audio)} samples (kept {len(audio)/audio_before_trim*100:.1f}%)")
        ...
        logger.info(f"Preprocessed audio saved: {preprocessed_path}, duration: {len(audio)/16000:.2f}s")
