// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Portuguese (`pt`).
class AppLocalizationsPt extends AppLocalizations {
  AppLocalizationsPt([String locale = 'pt']) : super(locale);

  @override
  String get appTitle => 'Bockarie';

  @override
  String get navHome => 'Início';

  @override
  String get navNewShipment => 'Novo Envio';

  @override
  String get navSettings => 'Configurações';

  @override
  String get settingsTitle => 'Configurações';

  @override
  String get settingsAppearance => 'Aparência';

  @override
  String get settingsThemeMode => 'Modo de Tema';

  @override
  String get settingsThemeLightTooltip => 'Modo Claro';

  @override
  String get settingsThemeSystemTooltip => 'Padrão do Sistema';

  @override
  String get settingsThemeDarkTooltip => 'Modo Escuro';

  @override
  String get settingsConfiguration => 'Configuração';

  @override
  String get settingsRateTables => 'Tabelas de Tarifas';

  @override
  String get settingsRateTablesSubtitle => 'Gerir tarifas de transportadoras';

  @override
  String get settingsAiProviders => 'Fornecedores de IA';

  @override
  String get settingsAiProvidersSubtitle => 'Configurar modelos de IA';

  @override
  String get settingsAbout => 'Sobre';

  @override
  String get settingsLanguageTitle => 'Idioma';

  @override
  String get settingsLanguageSubtitle => 'Selecionar idioma da aplicação';

  @override
  String get languageEnglish => 'Inglês';

  @override
  String get languageGerman => 'Alemão';

  @override
  String get languageChinese => 'Chinês';

  @override
  String get languageFrench => 'Francês';

  @override
  String get languageSpanish => 'Espanhol';

  @override
  String get languageItalian => 'Italiano';

  @override
  String get languagePortuguese => 'Português';

  @override
  String get languageDutch => 'Holandês';

  @override
  String get languagePolish => 'Polaco';

  @override
  String get languageGreek => 'Grego';

  @override
  String get languageCzech => 'Checo';

  @override
  String get languageHungarian => 'Húngaro';

  @override
  String get languageRomanian => 'Romeno';

  @override
  String get languageSwedish => 'Sueco';

  @override
  String get languageDanish => 'Dinamarquês';

  @override
  String get languageFinnish => 'Finlandês';

  @override
  String get languageSlovak => 'Eslovaco';

  @override
  String get languageBulgarian => 'Búlgaro';

  @override
  String get languageCroatian => 'Croata';

  @override
  String get languageLithuanian => 'Lituano';

  @override
  String get languageLatvian => 'Letão';

  @override
  String get languageSlovenian => 'Esloveno';

  @override
  String get languageEstonian => 'Estoniano';

  @override
  String get languageMaltese => 'Maltês';

  @override
  String get languageIrish => 'Irlandês';

  @override
  String get languageArabic => 'Árabe';

  @override
  String get languageTurkish => 'Turco';

  @override
  String get languageHebrew => 'Hebraico';

  @override
  String get languagePersian => 'Persa';

  @override
  String get systemDefaultLanguage => 'Padrão do Sistema';

  @override
  String get settingsCurrencyTitle => 'Currency';

  @override
  String get settingsCurrencySubtitle => 'Select display currency';

  @override
  String get currencyEuro => 'Euro';

  @override
  String get currencyUsd => 'US Dollar';

  @override
  String get currencyGbp => 'British Pound';

  @override
  String get searchHint => 'Pesquisar idiomas...';

  @override
  String get titleRecentShipments => 'Envios Recentes';

  @override
  String get titleNewShipment => 'Novo Envio';

  @override
  String get titleShipmentDetails => 'Detalhes do Envio';

  @override
  String get titleShippingQuotes => 'Cotações de Envio';

  @override
  String get titleAvailableQuotes => 'Cotações Disponíveis';

  @override
  String get titleQuoteComparison => 'Comparação de Cotações';

  @override
  String get titlePackingOptimizer => 'Otimizador de Embalagem';

  @override
  String get emptyStateNoShipments => 'Ainda sem envios';

  @override
  String get emptyStateCreateFirst => 'Crie o seu primeiro envio para começar';

  @override
  String get emptyStateNoQuotes => 'Nenhuma Cotação de Envio Disponível';

  @override
  String get labelFrom => 'De';

  @override
  String get labelTo => 'Para';

  @override
  String get labelOriginCity => 'Cidade de Origem';

  @override
  String get labelOriginPostal => 'Código Postal de Origem';

  @override
  String get labelDestinationCity => 'Cidade de Destino';

  @override
  String get labelDestinationPostal => 'Código Postal de Destino';

  @override
  String get labelPostalCode => 'Código Postal';

  @override
  String get labelCountry => 'País';

  @override
  String get labelState => 'Estado';

  @override
  String get labelCity => 'Cidade';

  @override
  String get labelNotes => 'Notas (opcional)';

  @override
  String get labelCartons => 'Caixas';

  @override
  String get labelWeight => 'Peso';

  @override
  String get labelLength => 'Comprimento (cm)';

  @override
  String get labelWidth => 'Largura (cm)';

  @override
  String get labelHeight => 'Altura (cm)';

  @override
  String get labelWeightKg => 'Peso (kg)';

  @override
  String get labelQuantity => 'Quantidade';

  @override
  String get labelItemType => 'Tipo de Item';

  @override
  String labelCartonNumber(int number) {
    return 'Caixa $number';
  }

  @override
  String get buttonSave => 'Guardar';

  @override
  String get buttonCancel => 'Cancelar';

  @override
  String get buttonDelete => 'Eliminar';

  @override
  String get buttonAddCarton => 'Adicionar Caixa';

  @override
  String get buttonSaveShipment => 'Guardar Envio';

  @override
  String get buttonViewQuotes => 'Ver Cotações';

  @override
  String get buttonOptimize => 'Otimizar';

  @override
  String get buttonBookThis => 'Reservar Este';

  @override
  String get buttonDetails => 'Detalhes';

  @override
  String get buttonLess => 'Menos';

  @override
  String get buttonBook => 'Reservar';

  @override
  String get buttonDiscard => 'Descartar';

  @override
  String get buttonSaveChanges => 'Guardar Alterações';

  @override
  String get buttonGotIt => 'Entendi';

  @override
  String get buttonResetToOriginal => 'Repor Original';

  @override
  String get buttonRecalculateQuotes => 'Recalcular Cotações';

  @override
  String get buttonApplySuggestion => 'Aplicar Sugestão';

  @override
  String get actionExportPdf => 'Exportar PDF';

  @override
  String get actionOptimizePacking => 'Otimizar Embalagem';

  @override
  String get tooltipBack => 'Voltar';

  @override
  String get tooltipHelp => 'Ajuda';

  @override
  String get tooltipSort => 'Ordenar';

  @override
  String get tooltipListView => 'Vista de Lista';

  @override
  String get tooltipGroupByTransportMethod =>
      'Agrupar por Método de Transporte';

  @override
  String get hintClickToEdit => 'Clique para editar';

  @override
  String get validationRequired => 'Obrigatório';

  @override
  String get validationInvalid => 'Inválido';

  @override
  String get errorAddCartons => 'Por favor adicione pelo menos uma caixa';

  @override
  String get errorSavingShipment => 'Erro ao guardar envio';

  @override
  String get errorLoadingShipments => 'Erro ao carregar envios';

  @override
  String get errorLoadingCartons => 'Erro ao carregar caixas';

  @override
  String get errorUpdatingAddress => 'Erro ao atualizar morada';

  @override
  String get errorExportingPdf => 'Erro ao exportar PDF';

  @override
  String get errorRecalculating => 'Erro ao recalcular cotações';

  @override
  String get errorSavingChanges => 'Erro ao guardar alterações';

  @override
  String get errorDeletingShipment => 'Erro ao eliminar envio';

  @override
  String get errorApplySuggestion => 'Erro ao aplicar sugestão';

  @override
  String get successShipmentSaved => 'Envio guardado com sucesso';

  @override
  String get successShipmentDeleted => 'Envio eliminado com sucesso';

  @override
  String get successPdfExported => 'PDF exportado com sucesso';

  @override
  String get successChangesSaved =>
      'Alterações guardadas! Cotações atualizadas.';

  @override
  String successAddressUpdated(int count) {
    return 'Morada atualizada! $count cotações geradas.';
  }

  @override
  String get successOptimizationApplied =>
      'Otimização aplicada! As cotações foram recalculadas.';

  @override
  String get statusCalculatingQuotes => 'A calcular cotações...';

  @override
  String get statusLoading => 'A carregar...';

  @override
  String get statusSavingShipment => 'A guardar envio e gerar cotações...';

  @override
  String get statusNoQuotesAvailable => 'Nenhuma cotação disponível';

  @override
  String get statusInTransit => 'In Transit';

  @override
  String get statusDelivered => 'Delivered';

  @override
  String get statusPending => 'Pending';

  @override
  String get deleteShipmentTitle => 'Eliminar Envio';

  @override
  String deleteShipmentMessage(String origin, String destination) {
    return 'Tem a certeza que deseja eliminar o envio de $origin para $destination?';
  }

  @override
  String get bookShipmentTitle => 'Reservar Envio';

  @override
  String bookShipmentMessage(String carrier, String service) {
    return 'Reservar envio com $carrier $service?';
  }

  @override
  String get bookingFeatureComingSoon => 'Funcionalidade de reserva em breve!';

  @override
  String get editOriginTitle => 'Editar Origem';

  @override
  String get editDestinationTitle => 'Editar Destino';

  @override
  String get editDimensionsTitle => 'Editar Dimensões';

  @override
  String get editDimensionsSubtitle =>
      'Experimente diferentes configurações de embalagem';

  @override
  String get autoFillNote =>
      'País, estado e código postal serão preenchidos automaticamente quando selecionar uma cidade';

  @override
  String get sortPriceLowHigh => 'Preço: Mais Baixo ao Mais Alto';

  @override
  String get sortPriceHighLow => 'Preço: Mais Alto ao Mais Baixo';

  @override
  String get sortSpeedFastest => 'Velocidade: Mais Rápido Primeiro';

  @override
  String get sortSpeedSlowest => 'Velocidade: Mais Lento Primeiro';

  @override
  String get filterAll => 'Todos';

  @override
  String get badgeCheapest => 'MAIS BARATO';

  @override
  String get badgeFastest => 'MAIS RÁPIDO';

  @override
  String get badgeBestInCategory => 'MELHOR DA CATEGORIA';

  @override
  String get quoteDetailsPriceBreakdown => 'Detalhamento de Preços';

  @override
  String get quoteDetailsChargeableWeight => 'Peso Taxável';

  @override
  String get quoteDetailsTotalPrice => 'Preço Total';

  @override
  String get quoteComparisonPotentialSavings => 'Poupança Potencial!';

  @override
  String get quoteComparisonCostIncrease => 'Aumento de Custo';

  @override
  String get quoteComparisonNoChange => 'Sem Alteração';

  @override
  String quoteComparisonAvailableQuotes(int count) {
    return 'Cotações Disponíveis ($count)';
  }

  @override
  String get quoteComparisonNoQuotesAvailable => 'Nenhuma Cotação Disponível';

  @override
  String get shippoTestLimitationMessage =>
      'As chaves API de teste do Shippo não suportam esta rota de envio.\n\nAs contas de transportadora de teste geralmente funcionam apenas para rotas domésticas dos EUA ou China-EUA.\n\nPara obter tarifas reais para esta rota, precisa de:\n• Mudar para chaves API do Shippo ao vivo\n• Conectar contas de transportadoras reais (UPS, DHL, FedEx, etc.)';

  @override
  String get shippoTestLimitationShort =>
      'As contas de transportadora de teste gratuitas do Shippo não suportam envios internacionais da China.';

  @override
  String get shippoHowToGetRealQuotes => 'Como obter cotações de envio reais:';

  @override
  String get shippoStep1 =>
      'Registe-se em contas de transportadoras (DHL, FedEx, UPS, etc.)';

  @override
  String get shippoStep2 => 'Conecte-as à sua conta Shippo';

  @override
  String get shippoStep3 =>
      'Atualize a sua chave API do Shippo nas configurações da aplicação';

  @override
  String get shippoInfoToGetRealQuotes => 'Para obter cotações de envio reais:';

  @override
  String get shippoInfoStep1 =>
      '1. Registe-se em contas de transportadoras (DHL, FedEx, UPS)';

  @override
  String get shippoInfoStep2 => '2. Conecte-as à sua conta Shippo';

  @override
  String get shippoInfoStep3 =>
      '3. Atualize a sua chave API nas configurações da aplicação';

  @override
  String get warningNoQuotesConfigureShippo =>
      'Morada atualizada! Nenhuma cotação disponível - configure contas de transportadora Shippo para tarifas reais.';

  @override
  String warningCouldNotGenerateQuotes(String error) {
    return 'Aviso: Não foi possível gerar cotações: $error';
  }

  @override
  String get optimizerCurrentPacking => 'Embalagem Atual';

  @override
  String get optimizerPackingSummary => 'Resumo da Embalagem';

  @override
  String get optimizerTotalCartons => 'Total de Caixas';

  @override
  String get optimizerActualWeight => 'Peso Real';

  @override
  String get optimizerDimensionalWeight => 'Peso Volumétrico';

  @override
  String get optimizerChargeableWeight => 'Peso Taxável';

  @override
  String get optimizerLargestSide => 'Maior Lado';

  @override
  String get optimizerOversizeWarning =>
      'Sobretamanho detetado - taxas extra aplicáveis';

  @override
  String get optimizerSuggestions => 'Sugestões de Otimização';

  @override
  String get optimizerCostSavingOpportunity => 'Oportunidade de Poupança';

  @override
  String get optimizerPackingOptimal => 'Embalagem parece ótima!';

  @override
  String get optimizerNoSuggestions =>
      'A sua embalagem atual é eficiente. Sem sugestões de otimização no momento.';

  @override
  String get optimizerCartonDetails => 'Detalhes da Caixa';

  @override
  String optimizerDimensions(double length, double width, double height) {
    final intl.NumberFormat lengthNumberFormat =
        intl.NumberFormat.decimalPattern(localeName);
    final String lengthString = lengthNumberFormat.format(length);
    final intl.NumberFormat widthNumberFormat =
        intl.NumberFormat.decimalPattern(localeName);
    final String widthString = widthNumberFormat.format(width);
    final intl.NumberFormat heightNumberFormat =
        intl.NumberFormat.decimalPattern(localeName);
    final String heightString = heightNumberFormat.format(height);

    return 'Dimensões: $lengthString×$widthString×$heightString cm';
  }

  @override
  String optimizerWeight(double weight) {
    final intl.NumberFormat weightNumberFormat =
        intl.NumberFormat.decimalPattern(localeName);
    final String weightString = weightNumberFormat.format(weight);

    return 'Peso: $weightString kg';
  }

  @override
  String optimizerItem(String type) {
    return 'Item: $type';
  }

  @override
  String get optimizerHelpTitle => 'Otimizador de Embalagem';

  @override
  String get optimizerHelpContent =>
      'O otimizador de embalagem analisa as suas caixas e sugere formas de reduzir custos de envio otimizando as dimensões.\n\nMétricas principais:\n• Peso Real: Peso físico dos itens\n• Peso Volumétrico: Calculado a partir do tamanho da caixa\n• Peso Taxável: O maior dos dois\n\nDica: Reduzir a altura da caixa geralmente proporciona as melhores poupanças!';

  @override
  String get liveTotalsUpdated => 'Totais Atualizados (Pré-visualização)';

  @override
  String get liveTotalsActual => 'Real';

  @override
  String get liveTotalsDim => 'Vol';

  @override
  String get liveTotalsChargeable => 'Taxável';

  @override
  String get liveTotalsInfoNote =>
      'Peso Vol = (C×L×A)/5000, Taxável = máx(Real, Vol) × Qtd. Clique em \"Recalcular Cotações\" para obter tarifas de envio reais da API Shippo.';

  @override
  String get liveTotalsOversizeWarning =>
      'Sobretamanho - taxas extra podem aplicar-se';

  @override
  String get transportExpressAir => 'Aéreo Expresso';

  @override
  String get transportStandardAir => 'Aéreo Padrão';

  @override
  String get transportAirFreight => 'Frete Aéreo';

  @override
  String get transportSeaFreightLCL => 'Frete Marítimo (LCL)';

  @override
  String get transportSeaFreightFCL => 'Frete Marítimo (FCL)';

  @override
  String get transportRoadFreight => 'Frete Rodoviário';

  @override
  String transportOptionsCount(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'opções',
      one: 'opção',
    );
    return '$count $_temp0';
  }

  @override
  String etaDays(int min, int max) {
    return '$min-$max dias';
  }

  @override
  String etaSingleDay(int days) {
    return '$days dias';
  }

  @override
  String weightKg(double weight) {
    final intl.NumberFormat weightNumberFormat =
        intl.NumberFormat.decimalPattern(localeName);
    final String weightString = weightNumberFormat.format(weight);

    return '$weightString kg';
  }

  @override
  String priceFrom(String price) {
    return 'a partir de $price';
  }

  @override
  String cheapestPrice(String weight, String price) {
    return '$weight kg • a partir de $price';
  }

  @override
  String routeDisplay(String origin, String destination) {
    return '$origin → $destination';
  }

  @override
  String cityPostalDisplay(String city, String postal) {
    return '$city, $postal';
  }

  @override
  String versionDisplay(String version, String buildNumber) {
    return 'Bockarie v$version ($buildNumber)';
  }

  @override
  String cheapestPriceChange(String oldPrice, String newPrice) {
    return 'Mais barato: $oldPrice → $newPrice';
  }

  @override
  String get instantQuotes => 'Cotações Instantâneas';

  @override
  String chargeableWeightDisplay(String weight) {
    return '$weight kg taxável';
  }

  @override
  String get badgeCheapestShort => 'Mais Barato';

  @override
  String get badgeFastestShort => 'Mais Rápido';

  @override
  String get badgeBestValue => 'Melhor Valor';

  @override
  String get quoteSubtotal => 'Subtotal';

  @override
  String get quoteFuelSurcharge => 'Taxa de Combustível';

  @override
  String get quoteOversizeFee => 'Taxa de Sobretamanho';

  @override
  String get optimizationFoundTitle => 'Otimização Encontrada!';

  @override
  String fewerCartons(int count) {
    return '$count caixa(s) a menos';
  }

  @override
  String lessChargeableWeight(String weight) {
    return '$weight kg de peso taxável a menos';
  }

  @override
  String get currentPackingOptimal => 'Embalagem atual já está otimizada';

  @override
  String get comparisonCurrent => 'Atual';

  @override
  String get comparisonOptimized => 'Otimizado';

  @override
  String get statCartons => 'Caixas';

  @override
  String get statChargeable => 'Taxável';

  @override
  String get statVolume => 'Volume';

  @override
  String get buttonApplyOptimization => 'Aplicar Otimização';

  @override
  String get buttonKeepOriginal => 'Manter Original';

  @override
  String get buttonClose => 'Fechar';

  @override
  String get helperSelectFromDropdown =>
      'Por favor selecione uma cidade da lista suspensa';

  @override
  String get validationSelectFromDropdown =>
      'Por favor selecione uma cidade da lista suspensa';
}
